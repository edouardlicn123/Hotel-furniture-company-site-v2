...
def series_add():
    ...
    try:
        ...
        if len(new_photos) < len(extra_files):
            current_app.logger.warning(f"Some images failed to upload: only {len(new_photos)} succeeded")
...
def series_edit(series_id):
    ...
    try:
        ...
        if remain_slots > 0:
            ...
            current_app.logger.info("Reached 5-image limit, additional uploads ignored")
