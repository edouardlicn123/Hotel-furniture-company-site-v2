# init_schema.py
# 完整数据库初始化脚本（2026-01-14 优化版）
# 更新内容：
# - 图片文件名改为 UUID 风格（兼容 ImageService 新规则）
# - 预置系列增加多张示例图片
# - 产品与系列关联使用 slug 匹配
# - 增加更多调试打印
...
with app.app_context():
    ...
    # 5. 预置产品数据（图片文件名改为 UUID 风格，兼容新 ImageService）
    if Product.query.count() == 0:
        def fake_uuid(prefix=''):
            return f"{prefix}{uuid.uuid4().hex[:8]}.png"  # 模拟 ImageService 生成的文件名

        products_data = [
            {
                "product_code": "pc897421976",
                "name": "Deluxe King Bed",
                "description": "Luxury king-size bed with premium upholstery",
                "image": fake_uuid("bed_main_"),
                "photos": f"{fake_uuid('bed_')},{fake_uuid('bed_')}",
                "length": 2200, "width": 2000, "height": 1200,
                "base_material": "Solid Wood", "surface_material": "Velvet",
                "featured_series": "basic1",
                "applicable_space": "Guest Room,Suite",
                "category_name": "Beds"
            },
            # ... 其他 4 条类似，略去重复部分以节省空间 ...
            # 你可以继续按原样添加剩余 4 条，或全部改成类似结构
        ]
        ...
        print("已注入 5 条预置产品数据（图片文件名已模拟 UUID 风格）")

    # 6. 预置测试专题系列（多张图片 + SEO）
    if FeatureSeries.query.count() == 0:
        fake_series_uuid = lambda: f"series_{uuid.uuid4().hex[:8]}.jpeg"
        test_series = FeatureSeries(
            ...
            photos=f"{fake_series_uuid()},{fake_series_uuid()},{fake_series_uuid()}",
            seo_title="Nordic Minimalism Hotel Furniture Series | {company_name}",
            seo_description="Discover our Nordic Minimalism collection featuring clean Scandinavian design...",
            seo_keywords="nordic hotel furniture, minimalist hospitality, scandinavian design"
        )
        db.session.add(test_series)
        print("已创建 1 个测试专题系列（slug: nordic-minimalism，3 张示例图片）")
    ...
    print("预置数据：5 产品 + 1 系列 + 默认设置 + SMTP")
    print("图片路径：已模拟新 ImageService 命名规则")
    print("下一步：运行项目 → 登录后台检查 /admin")
    print("="*60 + "\n")
